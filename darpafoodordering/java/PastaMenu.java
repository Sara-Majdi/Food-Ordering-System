...
public class PastaMenu extends AppCompatActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.pasta_menu);

        drawerLayout = findViewById(R.id.drawer);
        recyclerView = findViewById(R.id.menuRecyclerView);
        recyclerView.setLayoutManager(new LinearLayoutManager(PastaMenu.this));

        FirebaseRecyclerOptions<MenuHelperClass> options =
                new FirebaseRecyclerOptions.Builder<MenuHelperClass>()
                        .setQuery(FirebaseDatabase.getInstance().getReference().child("Menu").orderByChild("foodtype").equalTo("Pasta"), MenuHelperClass.class)
                        .build();
